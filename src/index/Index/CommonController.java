...
public class CommonController extends BaseController {
	...
	public Boolean init() {
		try {
			...
			if (session("userInfo") == null || session("userInfo").toString().equals("")) {
				Map<String, W> where = new HashMap<>();
				where.put(MEMBER.id, new W("eq", session("uid")));
				where.put(MEMBER.is_del, new W("eq", 0));
				Map<String, Object> user_info = new MU(MEMBER._table_name).where(where).find();
				if (user_info == null) {
					return false;
				} else {
					this.userInfo = JSONObject.parseObject(JSON.toJSONString(user_info));
					session("userInfo", this.userInfo);
				}
			} else {
				this.userInfo = (JSONObject) session("userInfo");
			}
			return true;
			...
			if (session("uid") == null || session("uid").toString().equals("")) {
				...
				if (I("code").equals("")) {
					...
					if (request.getQueryString() == null) {
						return_back_url = T.getRootUrl(request) + "/" + request.getRequestURI();
					} else {
						return_back_url = T.getRootUrl(request) + "/" + request.getRequestURI() + "?" + (request.getQueryString());
					}
					try {
