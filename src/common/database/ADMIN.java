			...
			if (map.containsKey(key)) {
				if (key.equals("mobile")) {
					if (data.get(key).toString().length() > 25) {
						return Common.error("管理员手机号最长25个字符");
					}
				}
				if (key.equals("username")) {
					if (data.get(key).toString().length() > 99) {
						return Common.error("管理员用户名最长99个字符");
					}
				}
				if (key.equals("password")) {
					if (data.get(key).toString().length() > 32) {
						return Common.error("管理员密码最长32个字符");
					}
				}
				...
				if (key.equals("group_id")) {
					if (T.toInt(data.get(key).toString()) > 2147483647) {
						return Common.error("所属角色id最大值2147483647");
					}
				...
				if (key.equals("is_del")) {
					if (T.toInt(data.get(key).toString()) > 2147483647) {
						return Common.error("是否删除（0 未删除；1 已删除）最大值2147483647");
					}
					if (data.get(key) == null || data.get(key).toString().trim().equals("")) {
						data.put(key, 0);
					}
