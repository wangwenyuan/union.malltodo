...
public class GoodsCategoryController extends BaseRenovation {
	public Boolean init() {
		Boolean b = super.init();
		if (b) {
			this.type = "Union/Index/category";
			this.platform = "mobile";
			this.assign("page_action", "商品列表页面");
			return true;
		} else {
			return false;
		}
	...
	public static JSONObject getGoodsListDom(String root, String rootUrl, JSONObject doms) throws IOException {
		JSONObject object = new JSONObject();
		JSONArray doms_sort = new JSONArray();
		for (String key : doms.keySet()) {
			if (doms.getJSONObject(key).getString("category").equals("union_categoryGoodsList_mobile")) {
