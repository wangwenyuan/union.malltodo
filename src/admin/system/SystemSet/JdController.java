...
public class JdController extends CommonController {
	public void indexPage() throws SQLException, IOException, ServletException, TemplateException {
		Map<String, Object> info = new MU(JD._table_name).order(JD.id + " desc").find();
		if (IS_POST) {
			Map<String, Object> data = Functions.I_TO_MAP(I());
			if (info == null) {
				new MU(JD._table_name).data(data).add();
			} else {
				Map<String, W> where = new HashMap<>();
				where.put(JD.id, new W("eq", info.get(JD.id)));
				new MU(JD._table_name).where(where).save(data);
			}
			this.success("设置成功");
		} else {
