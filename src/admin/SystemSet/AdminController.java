...
public class AdminController extends CommonController {
	public void indexPage() throws SQLException, IOException, ServletException, TemplateException {
		...
		List<Map<String, Object>> list = new MU(ADMIN._table_name).alias("a").join(ADMIN_GROUP._table_name, "as ag on a.group_id = ag.id", "left").where(where).order(ADMIN.id + " desc").field("a.*, ag." + ADMIN_GROUP.name + " as group_name").limit(page.firstRow + "," + page.listRows).select();
	...
	public void addPage() throws SQLException, IOException, ServletException, TemplateException {
		...
		map.put("0", "超级管理员");
		for (Integer i = 0; i < AdminGroupController.getAdminCategorys().size(); i = i + 1) {
			String _id = AdminGroupController.getAdminCategorys().get(i).get(ADMIN_GROUP.id).toString();
			String _name = AdminGroupController.getAdminCategorys().get(i).get(ADMIN_GROUP.name).toString();
			Integer level = T.toInt(AdminGroupController.getAdminCategorys().get(i).get("level").toString());
			_name = "—— " + _name;
		...
		if (IS_POST) {
			...
			// 检测选择的管理员组是否正确
			if (!map.containsKey(I(ADMIN.group_id).trim())) {
				this.error("不存在该岗位");
				return;
			}
			// 记入数据库
	...
	public void materialPage() throws SQLException, IOException, ServletException, TemplateException {
		...
		if (IS_POST) {
			...
			// 不能修改所属的管理员组
			if (data.containsKey(ADMIN.group_id)) {
				data.remove(ADMIN.group_id);
			}
			// 记入数据库
			if (id.equals("")) {
				new MU(ADMIN._table_name).data(data).add();
