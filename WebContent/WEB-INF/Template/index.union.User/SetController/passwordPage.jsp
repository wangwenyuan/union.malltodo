...
<div style="padding:1rem">
	<div>
		<div style="font-size:0.875rem; line-height:25px;">请输入旧密码：<font style="color:red">(如果未设置密码请留空)</font></div>
		<%=Common.template.htmlWidget("password_widget", "old_password", "") %>
		
		<div style="height:1rem"></div>
		
		<div style="font-size:0.875rem; line-height:25px;">请输入新密码：</div>
		<%=Common.template.htmlWidget("password_widget", MEMBER.password, "") %>
		
		<div style="height:1rem"></div>
		
		<div style="font-size:0.875rem; line-height:25px;">请再次输入新密码：</div>
		<%=Common.template.htmlWidget("password_widget", "repeat_password", "") %>
		...
		<script>
			var password_click_flag = 0;
			function tijiao(){
				if(password_click_flag == 1){
					return ;
				}else{
					password_click_flag = 1;
				}
				var shuju = {
				...
				http.post("<%=T.U("User/Set/password", "index.jsp", request) %>", shuju, function(data){
					layer.msg(data["info"], {
					    time: 2000
					}, function () {
						if(data["status"] == 0){
							password_click_flag = 0;
						}else{
