			...
			<td width="200px">
				<div id="renovation_menu_content">
					<div id="renovation_menu_box">
						...
						<%
						if(CONTROLLER_NAME.equals("Index") || CONTROLLER_NAME.equals("Custom")){
						%>
							<%@include file="./union/index_nav.jsp"%>
						<%
						}else if(CONTROLLER_NAME.equals("Bottom")){
						%>
							<%@include file="./union/bottom_nav.jsp"%>
						<%
						}else if(CONTROLLER_NAME.equals("Menu")){
						%>
							<%@include file="./union/menu_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("GoodsCategory")){
						%>
							<%@include file="./union/goods_category_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("GoodsDetail")){
						%>
							<%@include file="./union/goods_detail_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("Search")){
						%>
							<%@include file="./union/search_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("User")){
						%>
							<%@include file="./union/user_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("Team")){
						%>
							<%@include file="./union/team_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("MoneyLog")){
						%>
							<%@include file="./union/money_log_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("Order")){
						%>
							<%@include file="./union/order_nav.jsp"%>					
						<%
						}else if(CONTROLLER_NAME.equals("Income")){
						%>
							<%@include file="./union/income_nav.jsp"%>					
						<%
						}
						%>
					</div>
...
<script>
$(function() {
	$("#phone_body").sortable({tolerance: 'pointer', axis: 'y', cursorAt: 'top'});
});
