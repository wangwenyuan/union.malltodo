...
<html>
	<head>
		...
		<title>注册-MALLTODO</title>
		<meta name="keywords" content="注册，MALLTODO" />
	...
	<body style="background:#FFF;">
		...
		<div style="height:2.8125rem; text-align:center; line-height:2.8125rem; font-size:1rem; color:#666;"><span>注册</span></div>
		...
		<form action="" method="post" id="form_html">
			...
			<!-- <div class="input_box"><input class="input_text" style="border:none; border-bottom:#CCC 0.0625rem solid;" type="text"
				 name="recommend_code" id="recommend_code" placeholder="请输入推荐码" /></div> -->
			<div class="input_box">
				<input type="checkbox" id="xieyi" name="xieyi" /> <a onclick="show_agreement()" style="font-size:0.875rem; color:#333333;">注册协议</a>
			</div>
			<div class="input_box">
				<div class="input_text" style="text-align:center; background:#eee; border:#eee 0.0625rem solid;" id="denglu" onclick="register()">注册</div>
			</div>
		</form>
		
		<div class="input_box zhuce"><span style="float: left;"><a href="<%=T.U("Index/User/login", "index.jsp", request) %>">登录</a></span><span style="float: right;"><a href="<%=T.U("Index/User/find_password", "index.jsp", request) %>">找回密码</a></span></div>
		...
		<script>
			function load_verify_pic(){
				$("#send_sms_yanzheng").attr("src", "<%=T.U("Index/User/verify", "index.jsp", request) %>"+"&t="+Math.random());
			}
			load_verify_pic();
			$("#send_sms_yanzheng").click(function(){
				load_verify_pic();
			})
			send_sms("<%=T.U("Index/User/send_register_sms", "index.jsp", request) %>");
			
			var register_click_flag = 0;
			function register(){
				if(!$("#xieyi").prop('checked')){
					layer.msg("必须要同意协议才能注册");
					return ;
				}
				if(register_click_flag == 1){
					return ;
				}else{
				...
				var shuju = {
					...
					"code":$("#sms_code").val(),
					"xieyi":$("#xieyi").prop('checked')
				}
				http.post("<%=T.U("Index/User/register", "index.jsp", request) %>", shuju, function(data){
					layer.msg(data["info"], {
					    time: 2000
					}, function () {
					    if (data["url"] != "") {
			...
			function show_agreement(){
				layer.open({
					...
					maxmin: true,
					area: ['70%', '70%'],
					content: ["<%=T.U("Index/User/agreement", "index.jsp", request) %>"],
				});
			}
		</script>
	</body>
