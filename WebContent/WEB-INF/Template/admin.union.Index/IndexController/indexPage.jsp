		...
		<div class="box_div_6">
			<div class="box_div_title">
				...
				<span style="padding-left:15px; line-height:30px; font-size:16px; color:#333;">未处理的提现申请</span> <span class="span_btn span_select"
				 onclick="window.location.href = './admin.union.jsp?m=Finance&c=Index&a=index'">前去处理 </span>
			</div>
...
<script>
	...
	function get_sales_ranking_of_products(day_num) {
		...
		if (day_num == 7) {
			//说明是近7天数据
			sales_ranking_of_products_url = "<%=T.U("Index/IndexData/get_serven_day_sales_ranking_of_products", "admin.union.jsp", request) %>";
		} else if (day_num == 1) {
			//说明是今天数据
			sales_ranking_of_products_url = "<%=T.U("Index/IndexData/get_today_sales_ranking_of_products", "admin.union.jsp", request) %>";
		} else if (day_num == -1) {
			//说明是昨天数据
			sales_ranking_of_products_url = "<%=T.U("Index/IndexData/get_yesterday_sales_ranking_of_products", "admin.union.jsp", request) %>";
		}
